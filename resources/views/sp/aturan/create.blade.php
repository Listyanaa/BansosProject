...
<div class="sp-container">
    ...
    <div class="sp-toolbar">
        <div class="sp-title-group">
            <h3 class="sp-title">Tambah Aturan</h3>
        </div>
    ...
    <div class="sp-card-form">
        <form method="post" action="{{ route('sp.aturan.store') }}">
            @csrf
                ...
                @forelse($syarat as $s)
                    <label class="sp-syarat-item">
                        <input type="checkbox"
                               name="kondisi[]"
                               value="{{ $s->kode }}"
                               {{ (is_array(old('kondisi')) && in_array($s->kode, old('kondisi'))) ? 'checked' : '' }}>
                        <span class="sp-syarat-kode">{{ $s->kode }}</span>
                        <span class="sp-syarat-teks">— {{ $s->teks }}</span>
                    </label>
                @empty
                    <p class="sp-syarat-empty">
                        Belum ada data syarat. Silakan buat syarat terlebih dahulu.
                    </p>
                @endforelse
            </div>
            ...
            <div class="sp-radio-row">
                <label class="sp-radio">
                    <input type="radio" name="hasil" value="Layak"
                           {{ old('hasil','Layak') === 'Layak' ? 'checked' : '' }}>
                    <span>Layak</span>
                </label>
                <label class="sp-radio">
                    <input type="radio" name="hasil" value="Tidak Layak"
                           {{ old('hasil') === 'Tidak Layak' ? 'checked' : '' }}>
...
<style>
/* Container & header sama dengan submenu Syarat */
.sp-container{
    background:#fff;
    padding:22px;
    border-radius:14px;

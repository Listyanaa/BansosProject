...
<div class="sp-container">
    ...
    <div class="dashboard-header">
        <div class="header-content">
            <h1 class="header-title">Kelola Data Aturan</h1>
            <p class="header-subtitle">Kelola aturan menerima bantuan</p>
        </div>
    </div>

    <div class="sp-actions">
        <a href="{{ route('sp.aturan.create') }}" class="btn-add">
    ...
    @else
        <div class="table-container">
            <table id="tabel-aturan" class="display nowrap data-table" style="width:100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($items as $it)
                        <tr>
                            <td data-order="{{ $it->kode_aturan }}">
                                <span class="badge-kode">{{ $it->kode_aturan }}</span>
                            </td>

                            <td>{{ $it->kondisi }}</td>

                            <td style="text-align:center;" data-order="{{ $it->hasil === 'Layak' ? 1 : 0 }}">
                                @if($it->hasil === 'Layak')
                                    <span class="badge-status badge-yes">Layak</span>
                                @else
                                    <span class="badge-status badge-no">Tidak Layak</span>
                                @endif
                            </td>

                            <td style="white-space:nowrap;">
                                <div class="row-actions">
                                    <a href="{{ route('sp.aturan.edit',$it) }}" class="btn-edit">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                        <span>Edit</span>
                                    </a>

                                    <form method="post"
                                          action="{{ route('sp.aturan.destroy',$it) }}"
  ...
  if ($tbl.length) {
    const dt = $tbl.DataTable({
      ...
      initComplete: function () {
        ...
        $search.attr('placeholder', 'Kata kunci (kode/kondisi/hasil)');
      }
    });
